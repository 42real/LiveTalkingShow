...
`raw` payload 是 `RGBA8`，每个像素 4 字节，顺序为 `R,G,B,A`。`jpeg/png/webp` payload 是对应图片字节；`jpeg` 不保留透明，`png/webp` 可以保留透明。`width/height` 来自当前 avatar 的 `full_imgs` 画布尺寸；如果 avatar 是普通 BGR/BGRA 图，服务端会统一转换成 RGBA。没有透明通道的 avatar 会补全不透明 alpha。
...
| `format` | `/alpha/ws?format=jpeg` | `raw` / `jpeg` / `png` / `webp`。Web 预览推荐 `jpeg`，透明 overlay 推荐 `raw`。 |
| `quality` | `/alpha/ws?format=jpeg&quality=80` | `1-100`，只影响 `jpeg` / `webp`。 |
...
Web 调试页默认走 `jpeg` 压缩预览，适合远程浏览器和 VSCode 端口转发。overlay 默认也走 alpha stream，但使用 `raw` 保留透明通道。为了避免高分辨率 raw RGBA 卡顿，`testclient/.env.example` 默认让 overlay 连接 `/alpha/ws?max_height=1080&fps=15&format=raw`；如果必须无损原始帧，把 `LIVETALKING_VIDEO_MAX_HEIGHT` 和 `LIVETALKING_VIDEO_FPS` 设为 `0`。
...
| `VITE_ALPHA_VIDEO_QUALITY` | `1-100` | `80` | `jpeg` / `webp` 编码质量。 |
