...
| 远程透明 WebRTC | `POST <LiveTalking>/alpha/webrtc/packed_offer` | WebRTC answer。服务端发音频轨和 packed 视频轨；视频左半是 color，右半是 alpha，客户端本地合成透明画面。 |
| 远程透明 WebRTC 双轨 | `POST <LiveTalking>/alpha/webrtc/offer` | 对照接口。服务端发音频轨、彩色视频轨、alpha mask 视频轨；两条视频轨可能不同步。 |
| 中断 | `POST <LiveTalking>/interrupt_talk` | JSON sessionid。 |
| 关闭 | `POST <LiveTalking>/alpha/close` | JSON sessionid。 |

远程透明 WebRTC 客户端要用 `bundlePolicy: "max-bundle"`。推荐 packed 模式按 `audio recvonly`、`video recvonly` 的顺序创建 transceiver；单条视频轨左半是 color，右半是 alpha。旧双轨模式才需要第三个 `video recvonly`，但它可能出现 color/alpha 跨轨不同步。
